...
namespace Plato.Articles.Categories.Follow.Notifications
{
    ...
    public class NewArticleEmail : INotificationProvider<Article>
    {
        ...
        public async Task<ICommandResult<Article>> SendAsync(INotificationContext<Article> context)
        {
            ...
            if (!context.Notification.Type.Name.Equals(EmailNotifications.NewArticle.Name, StringComparison.Ordinal))
            {
                return null;
            }
            ...
            // Get email template
            const string templateId = "NewArticle";
