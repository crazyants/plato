...
{
    public class AnswerOperator : ISpamOperatorProvider<Answer>
    {
        
        private readonly IUserNotificationTypeDefaults _userNotificationTypeDefaults;
        private readonly INotificationManager<Answer> _notificationManager;
        ...
        public AnswerOperator(
            ...
            INotificationManager<Answer> notificationManager,
        ...
        public async Task<ISpamOperatorResult<Answer>> ValidateModelAsync(ISpamOperatorContext<Answer> context)
        {

            // Ensure correct operation provider
            if (!context.Operation.Name.Equals(SpamOperations.Answer.Name, StringComparison.Ordinal))
            {
                return null;
            }
            ...
            // Create result
            var result = new SpamOperatorResult<Answer>();
        ...
        public async Task<ISpamOperatorResult<Answer>> UpdateModelAsync(ISpamOperatorContext<Answer> context)
        {
        ...
        async Task NotifyAsync(ISpamOperatorContext<Answer> context)
        {
            ...
            foreach (var user in users)
            {
                ...
                // Web notification
                if (user.NotificationEnabled(_userNotificationTypeDefaults, WebNotifications.AnswerSpam))
                {
                    await _notificationManager.SendAsync(new Notification(WebNotifications.AnswerSpam)
                    {
                        To = user,
                        From = bot
                    }, context.Model);
                ...
                // Email notification
                if (user.NotificationEnabled(_userNotificationTypeDefaults, EmailNotifications.AnswerSpam))
                {
                    await _notificationManager.SendAsync(new Notification(EmailNotifications.AnswerSpam)
                    {
                        To = user
                    }, context.Model);
                }
