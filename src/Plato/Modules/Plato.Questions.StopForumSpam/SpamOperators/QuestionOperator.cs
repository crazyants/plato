...
namespace Plato.Questions.StopForumSpam.SpamOperators
{
    ...
    public class QuestionOperator : ISpamOperatorProvider<Question>
    {
        
        private readonly IUserNotificationTypeDefaults _userNotificationTypeDefaults;
        private readonly INotificationManager<Question> _notificationManager;
        ...
        public QuestionOperator(
            ...
            INotificationManager<Question> notificationManager,
        ...
        public async Task<ISpamOperatorResult<Question>> UpdateModelAsync(ISpamOperatorContext<Question> context)
        {
        ...
        async Task NotifyAsync(ISpamOperatorContext<Question> context)
        {
            ...
            foreach (var user in users)
            {
                ...
                // Web notification
                if (user.NotificationEnabled(_userNotificationTypeDefaults, WebNotifications.QuestionSpam))
                {
                    await _notificationManager.SendAsync(new Notification(WebNotifications.QuestionSpam)
                    {
                        To = user,
                        From = bot
                    }, context.Model);
                ...
                // Email notification
                if (user.NotificationEnabled(_userNotificationTypeDefaults, EmailNotifications.QuestionSpam))
                {
                    await _notificationManager.SendAsync(new Notification(EmailNotifications.QuestionSpam)
                    {
                        To = user
                    }, context.Model);
                }
