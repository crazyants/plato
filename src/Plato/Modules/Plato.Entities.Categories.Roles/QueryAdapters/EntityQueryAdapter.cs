...
namespace Plato.Entities.Categories.Roles.QueryAdapters
{
    ...
    public class EntityQueryAdapter : BaseQueryAdapterProvider<Entity> 
    {

        public override void BuildWhere(IQuery<Entity> query, StringBuilder builder)
        {

            // Ensure correct query type
            if (query.GetType() != typeof(EntityQuery<Entity>))
            {
                return;
            }

            // Convert to correct query type
            var q = (EntityQuery<Entity>)Convert.ChangeType(query, typeof(EntityQuery<Entity>));
