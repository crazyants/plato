...
namespace Plato.Entities.Categories.Roles.QueryAdapters
{
    ...
    public class FeatureEntityCountQueryAdapter : BaseQueryAdapterProvider<FeatureEntityCount>
    {

        public override void BuildWhere(IQuery<FeatureEntityCount> query, StringBuilder builder)
        {

            // Ensure correct query type
            if (query.GetType() != typeof(FeatureEntityCountQuery<FeatureEntityCount>))
            {
                return;
            }

            // Convert to correct query type
            var q = (FeatureEntityCountQuery<FeatureEntityCount>)Convert.ChangeType(query, typeof(FeatureEntityCountQuery<FeatureEntityCount>));
