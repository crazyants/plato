...
namespace Plato.Docs.StopForumSpam.SpamOperators
{
    ...
    public class DocOperator : ISpamOperatorProvider<Doc>
    {
        ...
        public async Task<ISpamOperatorResult<Doc>> ValidateModelAsync(ISpamOperatorContext<Doc> context)
        {

            // Ensure correct operation provider
            if (!context.Operation.Name.Equals(SpamOperations.Doc.Name, StringComparison.Ordinal))
            {
                return null;
            }
            ...
            // Create result
            var result = new SpamOperatorResult<Doc>();
        ...
        public async Task<ISpamOperatorResult<Doc>> UpdateModelAsync(ISpamOperatorContext<Doc> context)
        {
        ...
        async Task NotifyAsync(ISpamOperatorContext<Doc> context)
        {
            ...
            foreach (var user in users)
            {
                ...
                // Web notification
                if (user.NotificationEnabled(_userNotificationTypeDefaults, WebNotifications.DocSpam))
                {
                    await _notificationManager.SendAsync(new Notification(WebNotifications.DocSpam)
                    {
                        To = user,
                        From = bot
                    }, context.Model);
                ...
                // Email notification
                if (user.NotificationEnabled(_userNotificationTypeDefaults, EmailNotifications.DocSpam))
                {
                    await _notificationManager.SendAsync(new Notification(EmailNotifications.DocSpam)
                    {
                        To = user
                    }, context.Model);
                }
