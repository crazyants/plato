...
namespace Plato.Internal.Layout.LocationExpander
{
    ...
    ////                var viewsPath = "/Views/{{1}}/{{0}}" + RazorViewEngine.ViewExtension;
    ////                var sharedViewsPath = "/Views/Shared/{0}" + RazorViewEngine.ViewExtension;

    ////                moduleComponentViewLocations = _modulesWithComponentViews                        
    ////                    .Select(m => "~/Modules/" + m.Descriptor.Id + sharedViewsPath)
    ////                    .ToList();

    ////                moduleComponentViewLocations.AddRange(_modulesWithComponentViews                        
    ////                    .Select(m => "~/Modules/" + m.Descriptor.Id + viewsPath)
    ////                    .ToList());
